...
constexpr auto OUTPUT_FNAME = "sgdagc_clip0.1_lr1.0.txt";
...
void train(torch::nn::ModuleHolder<EfficientNetV1Impl>& net, Dataloader& data_loader, torch::optim::Optimizer& optimizer, size_t dataset_size) {
    /*
    ...
    for(int i=0; i<25; i++) {
        ...
        MyFile << Acc / dataset_size << std::endl;
...
int main(int argc, const char * argv[]) {
   ...
   auto netx = vision::models::ResNet34(3);
    ...
    //torch::nn::Linear lin(512, 3); // the last layer of resnet, which we want to replace, has dimensions 512x1000
...
//    torch::optim::SGD opt(net.get()->parameters(), torch::optim::SGDOptions(1e-3));
    ...
    train(net, data_loader, opt, custom_dataset.size().value());
